...
export class GenericListFilterComponent implements OnInit {
  ...
  onRemoveFilter(item) {
    if (item == 'time') {
      this.fromTime = format(subHours(new Date(), 3), 'h:mm aa');
      this.toTime = format(new Date(), 'h:mm aa');
  ...
  selectedFilterTab(type) {
    this.searchKey = '';
    let indx = this.selectedValues.findIndex((item) => {
  ...
  isFilterExists(filter) {
    ...
    // let indx = findIndex(this.selectedValues,);
    if (indx > -1) {
      return true;
    }
    return false
